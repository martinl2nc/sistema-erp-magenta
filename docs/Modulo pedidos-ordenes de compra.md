...
3. **Validación de Sustentos y Líneas (Modal Maestro):**
   ...
   * **Descuento Global Editable:** El descuento de la cotización se precarga, pero el vendedor puede editarlo.
4. **Procesamiento (Supabase):**
   ...
   * Se inserta un nuevo registro en la tabla `pedidos`, con los siguientes datos propios:
     * `cotizacion_id` (referencia opcional a la cotización de origen).
     * `cliente_id` (referencia directa al cliente).
     * `numero_pedido` (identificador secuencial: PED-1, PED-2, etc.).
     * **Datos financieros calculados**: `aplica_igv`, `subtotal`, `descuento_global_monto`, `igv_monto`, `total_final`.
     * URL del archivo de sustento.
...
> **Nota Arquitectónica:** Los datos financieros del pedido se almacenan directamente en la tabla `pedidos`, independientes de la cotización de origen. Esto permite que un pedido exista sin cotización previa y que los totales reflejen el pedido real (con cantidades y descuentos ajustados).
