...
type Tab = 'cobranzas' | 'transacciones' | 'aging';
  ...
  return (
    <div className="max-w-7xl w-full mx-auto flex flex-col md:h-full">
      ...
      <div className="flex border-b border-[#334155] mb-6 gap-1">
        {([['cobranzas', 'Cobranzas'], ['transacciones', 'Transacciones'], ['aging', 'x Antigüedad']] as [Tab, string][]).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
      ...
      {/* Tab: Transacciones */}
      {tab === 'transacciones' && <ListadoCobros embedded />}

      {/* Tab: Antigüedad */}
      {tab === 'aging' && <AgingReport embedded />}
