...
import { useAgingReportCompras, useAgingDetalleCompras } from '@/hooks/usePagosEmitidos';
import { formatCurrency, formatDate, getClientDisplayName, formatLongDate } from '@/utils/formatters';
import type { AgingDetalleComprasRow, AgingReportComprasRow } from '@/services/pagos-emitidos.service';
...
function AgingDetalleRows({ proveedorId }: { proveedorId: string }) {
  const { data, isLoading } = useAgingDetalleCompras(proveedorId);
  ...
  return (
    <>
      ...
      <tr className="bg-[#0F1115]/80">
        ...
        <td className="px-4 py-2 text-[10px] font-bold tracking-wider uppercase text-[#94A3B8]">F. Vencimiento</td>
      ...
      {(data ?? []).map((row: AgingDetalleComprasRow) => (
...
interface AgingReportComprasProps {
  embedded?: boolean;
}

export default function AgingReportCompras({ embedded = false }: AgingReportComprasProps) {
  const { data: rows, isLoading, isError } = useAgingReportCompras();
  ...
  const pctVencidoStr = totals.deuda > 0 ? `${((totalVencido / totals.deuda) * 100).toFixed(0)}%` : '0%';

  const bucketPcts = BUCKETS.reduce((acc, b) => {
    acc[b.key] = totals.deuda > 0 ? `${((totals[b.key] / totals.deuda) * 100).toFixed(0)}%` : '0%';
    return acc;
  }, {} as Record<BucketKey, string>);

  const rowCount = (rows ?? []).length;
  const proveedorLabel = `${rowCount} proveedor${rowCount !== 1 ? 'es' : ''}`;
    ...
    <div className={embedded ? 'flex flex-col flex-1' : 'max-w-7xl w-full mx-auto flex flex-col'}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          ...
          {!isLoading && !isError && (rows ?? []).length > 0 && (
            <div className="text-sm text-[#94A3B8] self-end sm:self-auto text-right">
              <span className="text-red-400 font-semibold">{pctVencidoStr}</span> de la deuda está vencida
            </div>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        <div className="bg-[#181B21] border border-[#334155] rounded-lg p-4 col-span-2 sm:col-span-1">
          ...
          <p className="text-xs text-[#94A3B8] mt-1">{proveedorLabel}</p>
        </div>
        {BUCKETS.map(b => (
          <div key={b.key} className="bg-[#181B21] border border-[#334155] rounded-lg p-4">
            ...
            {totals.deuda > 0 && (
              <p className="text-xs text-[#94A3B8] mt-1">{bucketPcts[b.key]}</p>
            )}
          </div>
        ))}
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[900px]">
              ...
              <tbody className="divide-y divide-[#334155] bg-[#181B21]">
                {(rows ?? []).map((row: AgingReportComprasRow) => {
                  const isExpanded = expandedIds.has(row.proveedor_id);
                  ...
                  return (
                    <React.Fragment key={row.proveedor_id}>
                      <tr
                        onClick={() => toggle(row.proveedor_id)}
                      ...
                      {isExpanded && <AgingDetalleRows proveedorId={row.proveedor_id} />}
                    </React.Fragment>
                  );
                })}
              </tbody>
